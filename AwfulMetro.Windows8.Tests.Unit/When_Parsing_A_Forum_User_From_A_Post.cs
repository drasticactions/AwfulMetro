...
namespace AwfulMetro.Windows8.Tests.Unit
{
    ...
    public class When_Parsing_A_Forum_User_From_A_Post
    {
        ...
        {
            ForumUserEntity user = GetParsedEntity(DefaultPostHtml);
        ...
        {
            ForumUserEntity user = GetParsedEntity(PostWithCustomTitle);
        ...
        {
            ForumUserEntity user = GetParsedEntity(PostWithNoAvatar);
        ...
        public void AvatarLink_Is_Parsed_When_Image_Exists()
        {
            ForumUserEntity user = GetParsedEntity(DefaultPostHtml);
            Assert.AreEqual(
                "<!DOCTYPE html><html><head><link rel=\"stylesheet\" type=\"text/css\" href=\"ms-appx-web:///Assets/ui-light.css\"></head><body style=\"background-color: white;\"></head><body><img src=\"http://fi.somethingawful.com/customtitles/title-the_dave-2.gif\" alt=\"\" class=\"img\" border=\"0\"></body></html>",
                user.AvatarLink);
        }
