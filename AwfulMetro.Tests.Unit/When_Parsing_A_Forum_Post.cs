...
namespace AwfulMetro.Tests.Unit
{
    ...
    public class When_Parsing_A_Forum_Post
    {
        ...
        {
            var post = ParseEntity(PostHtmlFromRegularUser);
        ...
        {
            var post = ParseEntity(PostHtmlFromRegularUser);
            Assert.AreEqual("<!-- BeginContentMarker --> <!-- google_ad_section_start --> The fact that this thread stands alone as a bulwark of non-terrible posting in the veritable wasteland that is new GBS is making me want to do this but I am super poor and super uncreative.  I am debating what to do. <!-- google_ad_section_end --> <!-- EndContentMarker -->   <p class=\"editedby\">", post.PostFormatted.Trim());
        }
