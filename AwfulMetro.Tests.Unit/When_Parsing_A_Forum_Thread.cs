...
namespace AwfulMetro.Tests.Unit
{
    ...
    public class When_Parsing_A_Forum_Thread
    {
        ...
        {
            var thread = ParseEntity(StickyLockedThread);
        ...
        {
            var thread = ParseEntity(LockedThread);
        ...
        {
            var thread = ParseEntity(ThreadWithUnreadPosts);
        ...
        {
            var thread = ParseEntity(PreviouslyViewedThreadWithNoNewPosts);
            Assert.IsTrue(thread.CanMarkAsUnread);
        }
        ...
        {
            var thread = ParseEntity(DefaultThread);
            Assert.AreEqual("http://fi.somethingawful.com/forums/posticons/icons-08/drugs.png#122", thread.ImageIconLocation);
        }
